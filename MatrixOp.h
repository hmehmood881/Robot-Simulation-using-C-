...
void Input_points(int no, double **pose)
{
	for (int i = 0; i < no; i++)
	{
		if (i == 0)
		{
			cout << "Please input the starting points (px, py, pz, phi) " << endl;
		}
		else if (i == (no - 1))
		{
			cout << "Please input the goal points " << endl;
		}
		else
		{

			cout << "Please input the " << i << " intermediate point " << endl;
		}
		int j = 0;
		for (int j = 0; j < 4; j++)
		{
...
void Display_Matrix(double **Mat, int rw, int clm)
{
	for (int i = 0; i < rw; i++)
	{
		cout << " [ ";
